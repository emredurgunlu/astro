...
export default function EnJsonEditor() {
  ...
  useEffect(() => {
    ...
    const loadJsonData = async () => {
      try {
        const response = await fetch("/api/json/en");
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen p-8">
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">EN JSON Düzenleyici</h1>
          <p className="text-gray-500">İngilizce dil dosyasını düzenleyin</p>
        </div>
        <Link href="/">
